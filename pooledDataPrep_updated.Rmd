...
context_plot_data<-plot_data %>%
  ...
  transmute(
    sample, contextValsSum=sum(ContextVals, na.rm=TRUE)
  ) %>%
  dplyr::distinct()  %>%
  ungroup %>%
  group_by(sample) %>%
  mutate(
    contextValsProp=(contextValsSum/sum(contextValsSum))*100
  )

context_plot_data$sampleFactor<-as.factor(context_plot_data$sample)

context_plot_data$Context[context_plot_data$Context == 'alone'] <- 'Alone'
context_plot_data$Context[context_plot_data$Context == 'closeTies'] <- 'Close Ties'
context_plot_data$Context[context_plot_data$Context == 'distantTies'] <- 'Distant Ties'
context_plot_data$Context[context_plot_data$Context == 'familyTies'] <- 'Family Ties'
context_plot_data$Context[context_plot_data$Context == 'home'] <- 'Home'
context_plot_data$Context[context_plot_data$Context == 'home_gym'] <- 'Gym'
context_plot_data$Context[context_plot_data$Context == 'home_nature'] <- 'Nature'
context_plot_data$Context[context_plot_data$Context == 'home_religion'] <- 'Religion'
context_plot_data$Context[context_plot_data$Context == 'home_social'] <- 'Social Places'
context_plot_data$Context[context_plot_data$Context == 'home_study'] <- 'Study Places'
context_plot_data$Context[context_plot_data$Context == 'home_transit'] <- 'Transit'
context_plot_data$Context[context_plot_data$Context == 'home_work'] <- 'Workplaces'



p2<-ggplot(data=context_plot_data, aes(x=reorder(Context, +contextValsProp), y=contextValsProp, group=sampleFactor, color=sampleFactor)) +  geom_point(size=8)+theme_apa() + xlab("Context") + ylab("Proportion") + theme(
